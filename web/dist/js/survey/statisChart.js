/**
 * Created by clouder on 16-10-12.
 */
$(function () {
    ...
    $.ajax({
        ...
        success:function (data) {
            if(data.survey.questionnaireSurvey.status == 2){
                $("#overSurveyBtn").attr("disabled",true);
                $("#overSurveyBtn").text("该调查问卷已结束");
            }
            var topic = data.survey.topic;
            $("#surveyTitle").text("问卷标题："+data.survey.questionnaireSurvey.title);
            $("#questionList").append(
                '<input type="hidden" id="surveyId" value="'+data.survey.questionnaireSurvey.id+'">'
            );
            var num = 1;
            /**遍历题目*/
...
function overSurveyBtn() {
    var id = $("#surveyId").val();
    if(confirm("是否结束此份调查问卷")){
        $.ajax({
            url:'overSurvey?id='+id,
            success:function (data) {
                alert("已成功结束该调查问卷");
                $("#overSurveyBtn").attr("disabled",true);
                $("#overSurveyBtn").text("该调查问卷已结束");
